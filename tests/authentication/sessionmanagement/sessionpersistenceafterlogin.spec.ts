...
test.describe('Authentication @S4pdub91w', () => {
  test('Session Persistence After Login @Tqxusmt2r', async ({ page }) => {
    ...
    const dashboardHeading = page.locator('h1, [data-testid="page-title"], [role="heading"]').filter({ hasText: textRegex('Dashboard') }).first();
    await safeExpectVisible(dashboardHeading, 'Dashboard heading not visible after login');

    // STEP 4: Refresh the page
    await page.reload();
    await waitForPageReady(page, '/dashboard');

    // STEP 5: Verify user remains logged in and Dashboard still accessible
    await expect(page).not.toHaveURL(/\/login/i, { timeout: 20000 });
    await safeExpectVisible(dashboardHeading, 'Dashboard heading not visible after refresh');
